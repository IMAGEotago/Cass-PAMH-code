...
# intrinsic <- read.csv("C:/Users/Alan/OneDrive - University of Otago/2022/Thesis/Dissertation Data/Data Files for R/Intrinsic Motivations, Personality Traits and Mental Health Survey_December 26 editing .csv")
...
# IPM <- read.csv("C:/Users/Alan/OneDrive - University of Otago/2022/Thesis/Dissertation Data/Data Files for R/IPM_Export.csv")
...
intrinsic.7 <- intrinsic %>% ##This %>% thing is called a pipe and it makes stacking commands on top of each other easy
  mutate(id = substr(intrinsic$ResponseId,1,7)) ##Extracts first 7 characters of response id so id column is same format as IPM data

##This will join by the ID in each data set but will keep all observations, even those that don't have a match
combined.all <- full_join(intrinsic.7, IPM, by = join_by(id == X))

##This only keeps observations that match up between datasets. You may want to check that all the ids are correct
##because this is case sensitive etc.
